...
import { SanteIndActivity } from '@/lib/firebase-sante-ind';
...
export function TimelineComponent() {
  ...
  const daysData = useMemo(() => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const anCount = dayActivities.filter(a => a.actType === 'affaire_nouvelle').length;
      const revisionCount = dayActivities.filter(a => a.actType === 'revision').length;
                    ...
                    <div className="text-center">
                      ...
                      {dayData.totalActs > 0 && (
                        <div className="text-xs mt-1">
                          ...
                          <div>Révisions: {dayData.revisionCount}</div>
                        </div>
                      )}
                    </div>
                  </TooltipContent>
          ...
          <DialogContent className="max-w-2xl">
            ...
            {selectedDay.activities.length === 0 ? (
              <p className="text-muted-foreground text-center py-4">
                Aucun acte santé individuelle ce jour
              </p>
            ) : (
                  ...
                  <div key={activity.id} className="p-3 border rounded-lg">
                    ...
                    <div className="flex gap-2 mt-2">
                      <Badge variant="secondary" className="text-xs">
                        Prime: {new Intl.NumberFormat('fr-FR', {
                          style: 'currency',
                          currency: 'EUR',
                          minimumFractionDigits: 0,
                          maximumFractionDigits: 0,
